...
function cplexSolve(t::Array{Int, 2})
    ...
    # CF : 
    @variable(m, domino[1:n+1, 1:n+2, 1:5], Bin)
    ...
    # Les deux chiffres du domino sont dominotés ensemble
        # Contraintes intérieures
            for i in 2:n
                for j in 2:n+1
                    @constraint(m, domino[i, j, 1] == domino[i, j-1, 2]) 
                    @constraint(m, domino[i, j, 2] == domino[i, j+1, 1]) 
        ...
        # Contraintes Bords haut et bas
            for j in 2:n+1
                @constraint(m, domino[0, j, 1] == domino[0, j-1, 2]) 
        ...
        # Contraintes Bords droit et gauche
            for i in 2:n
                @constraint(m, domino[i, 0, 2] == domino[i, 1, 1])
    ...
    # Objectif
    @objective(m, Max, sum(sum(sum(domino[i, j, k] for k in 2:5) for)
